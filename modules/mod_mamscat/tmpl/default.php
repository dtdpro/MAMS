...
if ($params->get('show_image',0)) {
	...
	if ($params->get('link_image',0)) echo '<a href="'.JRoute::_("index.php?option=com_mams&view=artlist&layout=category&catid=".$catinfo->cat_id.":".$catinfo->cat_alias).'">';
	if ($params->get('override_image','')) echo '<img src="'.$params->get('override_image').'" class="mams-featmod-image-img">';
	else echo '<img src="'.$catinfo->cat_image.'" class="mams-featmod-image-img">';
	if ($params->get('link_image',0)) echo '</a>';
	echo '</div>';
}
$firstart=true;
if ($articles) {
                    ...
                    if ($a->art_fielddata->$fn || $f->data) {
                        ...
                        switch ($f->field_type) {
                            ...
                            case "auths":
                                $auts = Array();
                                foreach ($f->data as $d) {
                                    if ($params->get('link_pubinfo', 0)) {
                                        $auts[] = '<a href="' . JRoute::_("index.php?option=com_mams&view=author&secid=" . $d->auth_sec . "&autid=" . $d->auth_id . ":" . $d->auth_alias) . '" class="mams-featmod-' . $f->group_name . '-' . $f->field_name . '-autlink">' . $d->auth_fname . (($d->auth_mi) ? " " . $d->auth_mi : "") . " " . $d->auth_lname . (($d->auth_titles) ? ", " . $d->auth_titles : "") . '</a>';
                                    } else {
                                        $auts[] = $d->auth_fname . (($d->auth_mi) ? " " . $d->auth_mi : "") . " " . $d->auth_lname . (($d->auth_titles) ? ", " . $d->auth_titles : "");
                                    }
                                }
                                echo implode(", ", $auts);
                                break;
