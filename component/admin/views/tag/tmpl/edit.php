...
<script type="text/javascript">
    ...
    {
        if (task == 'tag.cancel' || document.formvalidator.isValid(document.getElementById('mams-form'))) {
            Joomla.submitform(task, document.getElementById('mams-form'));
        }
    }
</script>
<form action="<?php echo JRoute::_('index.php?option=com_mams&layout=edit&tag_id='.(int) $this->item->tag_id); ?>" method="post" name="adminForm" id="mams-form" class="form-validate">
	<?php if (JVersion::MAJOR_VERSION == 4) { ?><div class="form-horizontal main-card"><?php } ?>
		...
		if (JVersion::MAJOR_VERSION == 4) {
			...
			echo HTMLHelper::_('uitab.addTab', 'myTab', 'general', Text::_('COM_MAMS_TAG_DETAILS'));
		} else {
			echo JHtml::_('bootstrap.startTabSet', 'myTab', array('active' => 'general'));
			echo JHtml::_('bootstrap.addTab', 'myTab', 'general', JText::_('COM_MAMS_TAG_DETAILS', true));
		}
		?>
