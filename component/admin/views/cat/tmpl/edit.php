...
<script type="text/javascript">
    ...
    {
        if (task == 'cat.cancel' || document.formvalidator.isValid(document.getElementById('mams-form'))) {
...
<form action="<?php echo JRoute::_('index.php?option=com_mams&layout=edit&cat_id='.(int) $this->item->cat_id); ?>" method="post" name="adminForm" id="mams-form" class="form-validate">
    <?php if (JVersion::MAJOR_VERSION == 4 || JVersion::MAJOR_VERSION == 5) { ?><div class="form-horizontal main-card"><?php } ?>
		...
		if (JVersion::MAJOR_VERSION == 4 || JVersion::MAJOR_VERSION == 5) {
			...
			echo HTMLHelper::_('uitab.addTab', 'myTab', 'general', Text::_('COM_MAMS_CAT_DETAILS'));
		} else {
			echo JHtml::_('bootstrap.startTabSet', 'myTab', array('active' => 'general'));
			echo JHtml::_('bootstrap.addTab', 'myTab', 'general', JText::_('COM_MAMS_CAT_DETAILS', true));
		}
		?>
