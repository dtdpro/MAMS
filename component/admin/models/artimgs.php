...
class MAMSModelArtimgs extends JModelList
{
	...
	protected function getListQuery() 
	{
		...
		if (is_numeric($artId)) {
			$query->where('a.ai_art = '.(int) $artId);
		}
		
		// Join over the fields.
		$query->select('f.field_title');
		$query->join('LEFT', '#__mams_article_fields AS f ON f.field_id = a.ai_field');
		
		//Filter by Field
		$field = $this->getState('filter.field');
		if (is_numeric($field))	$query->where('a.ai_field = '.(int) $field);

		// Join over the authors.
		$query->select('img.img_inttitle');
		$query->join('LEFT', '#__mams_images AS img ON img.img_id = a.ai_image');
